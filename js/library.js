...
this.actions={add:new this.Item(`plus-circle`,`Add Icon`),json:new this.Item(`code-json`,`JSON Object`),svg:new this.Item(`svg`,`SVG Sprite`),polymer:new this.Item(`polymer`,`HTML for Polymer`),import:new this.Item(`file-import`,`Import Library`),export:new this.Item(`file-export`,`Export Library`),clear:new this.Item(`delete`,`Clear Library`)};
...
if(!info.custom)info.actions.library.lastChild.nodeValue=`Add to Library`;
...
icons.array=Object.values(icons.use=icons.list=await(await fetch(`https://petershaggynoble.github.io/MDI-Sandbox/json/icons.json`)).json());
if(page.light)icons.array=Object.values(icons.use=await(await fetch(`https://petershaggynoble.github.io/MDI-Sandbox/json/light.json`)).json());
page.init();
