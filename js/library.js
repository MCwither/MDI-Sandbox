...
this.header.addEventListener(`click`,event=>{if(event.target.nodeName.toLowerCase()===`a`&&this.offline){this.alert(`不再可用。`);
...
}if(message)this.alert(`${message} 已复制到您的剪贴板。`);
...
break;
case this.actions.link:if(this.custom)page.alert(`不可用。`);
else if(this.retired)page.alert(`很长时间不可用。`);
else if(!page.offline)location.href=`https://materialdesignicons.com/icon/${this.name}`;

else page.alert(`不再可用。`);
break;
...
else if(this.retired)page.alert(`不可用。`);
else if(this.custom)page.alert(`不可用。`);
else page.alert(`不可用。`);
}},0);
...
icons.array=Object.values(icons.use=icons.list=await(await fetch(`${page.light?`../`:`https://petershaggynoble.github.io/MDI-Sandbox/`}json/icons.json`)).json());
